...
// applyVarsToStepFlags converts the OpenTofu vars specified in YAML into a list of -var flags
// with the variable value set in a format that OpenTofu expects.
func applyVarsToStepFlags(step *Step) error {
	...
	if err != nil {
		return fmt.Errorf("error marshaling OpenTofu variables to json")
	}

	parsedVars := gjson.Parse(string(vars))
	parsedVars.ForEach(func(key, value gjson.Result) bool {
		// ensure that the flag value is set using a format that OpenTofu expects
...
type Instruction struct {
	...
	OpenTofuFields `yaml:",inline"`
}

// OpenTofuFields represent fields specific to OpenTofu
type OpenTofuFields struct {
