...
func TestMixin_ValidatePayload(t *testing.T) {
	testcases := []struct {
		...
		{"install.missing-desc", "testdata/bad-install-input.missing-desc.yaml", false, "install.0.tofu: Invalid type. Expected: object, given: null"},
		{"install.desc-empty", "testdata/bad-install-input.desc-empty.yaml", false, "install.0.tofu.description: String length must be greater than or equal to 1"},
		{"upgrade.disable-var-file", "testdata/bad-upgrade-disable-save-var.yaml", false, "upgrade.0.tofu: Additional property disableVarFile is not allowed"},
		{"uninstall.input-not-valid", "testdata/bad-uninstall-input.input-not-valid.yaml", false, "uninstall.0.tofu: Additional property input is not allowed"},
		{"uninstall.disable-var-file", "testdata/bad-uninstall-disable-save-var.yaml", false, "uninstall.0.tofu: Additional property disableVarFile is not allowed"},
	}
...
func TestMixin_CheckSchema(t *testing.T) {
	...
	t.Run("mixin configuration", func(t *testing.T) {
		...
		_, err = jsonpath.Get("$.properties.tofu.properties.clientVersion", configSchema)
		require.NoError(t, err, "clientVersion was not included in the mixin config schema")
		_, err = jsonpath.Get("$.properties.tofu.properties.initFile", configSchema)
		require.NoError(t, err, "initFile was not included in the mixin config schema")
		_, err = jsonpath.Get("$.properties.tofu.properties.workingDir", configSchema)
